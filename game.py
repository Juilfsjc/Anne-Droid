...
pos_8 = 7
pos_9 = 8
REACTIONEMOJI = ["1️", "2️", "3️", "4️", "5️", "6️", "7️", "8️", "9️", "🛑"]


async def Games(ctx, bot):
    embed = discord.Embed(
    ...
    await msg.add_reaction('2️')

    def checkReaction(reaction, user):
        return user != bot.user and (str(reaction.emoji) == '1️' or str(reaction.emoji) == '2️')

    reaction, user = await bot.wait_for("reaction_add", timeout=30.0, check=checkReaction)
    if str(reaction.emoji) == '1️':
        await ticTacToe(ctx, bot)
        pass
    elif str(reaction.emoji) == '2️':
        # Call Rockpaperscissors
        pass


async def ticTacToe(ctx, bot):
    emojis = ["1️", "2️", "3️", "4️", "5️", "6️", "7️", "8️", "9️", "🛑"]
    ...
    while checkWin(player_1, player_2, board) == BLANK and turn <= 9:
        ...
        if winner != BLANK:
            ...
            await ctx.send(printBoard(player_1, player_2, board))
